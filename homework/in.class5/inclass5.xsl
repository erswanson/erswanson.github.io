...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
        ...
        <xsl:result-document href="{concat('cats/', name, '.html')}">
            <html>
                ...
                <body>
                    ...
                    <xsl:apply-templates select="name-in-wiki"/>
    ...
    <xsl:template match="wiki">
        <xsl:value-of select="concat('https://nekoatsume.fandom.com', @href)"/>
    </xsl:template>
    
    <xsl:template match="name-in-wiki">
    ...
    <xsl:template match="power-level">
        <xsl:element name="p"><xsl:text>My power level is </xsl:text><xsl:apply-templates/><xsl:text>.</xsl:text></xsl:element>
    </xsl:template>
    
    <xsl:template match="memento">
        <xsl:element name="p"><xsl:text>I am known for having a </xsl:text><xsl:apply-templates/><xsl:text>with me.</xsl:text></xsl:element>
    </xsl:template>
